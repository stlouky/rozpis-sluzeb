...
- Úkol 10 (deploy) — připraveno v `DEPLOY.md` (lokální, negitované),
  čeká na úkoly 1–9(b) hotové lokálně. DALŠÍ NA ŘADĚ (hlavní tok 1-9b
  je teď hotový).
...
## ✅ Migrace na `data/rozpis.db` provedena (úkol 9b, `nedostupnost.stav`)

Stejný vzor jako `zakaz_smeny`/`max_za_sebou` (ADD COLUMN, ne přetvoření
tabulky - CHECK na NOVÉM sloupci to v SQLite 3.25+ zvládne). Ověřeno:
`PRAGMA integrity_check` = ok, `foreign_key_check` = [], všech 19
existujících řádků má po migraci `stav='schvaleno'` (výchozí hodnota -
beze změny chování). Záloha: `data/rozpis.db.pred-migraci-stav-pozadavku.bak`.
